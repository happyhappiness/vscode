...
+            sdstolower(config.tests);
             ...
             exit_status = 0;
             goto usage;
@@ -398,24 +415,38 @@ int parseOptions(int argc, const char **argv) {
...
@@ -424,13 +455,28 @@ int showThroughput(struct aeEventLoop *eventLoop, long long id, void *clientData
     REDIS_NOTUSED(id);
...
+
 int main(int argc, const char **argv) {
     int i;
     char *data, *cmd;
@@ -451,13 +497,15 @@ int main(int argc, const char **argv) {
     config.randomkeys = 0;
     config.randomkeys_keyspacelen = 0;
     config.quiet = 0;
+    config.csv = 0;
