...
+        !strcasecmp(command,"psubscribe")) config.pubsub_mode = 1;
     ...
     /* Select db number */
@@ -390,18 +252,17 @@ static int cliSendCommand(int argc, char **argv, int repeat) {
         return 1;
     }
...
@@ -415,7 +276,6 @@ static int cliSendCommand(int argc, char **argv, int repeat) {
         }
...
@@ -453,6 +313,8 @@ static int parseOptions(int argc, char **argv) {
             i++;
...
@@ -479,7 +341,7 @@ static sds readArgFromStdin(void) {
...
+  char **sds = zmalloc(sizeof(char*)*count);
   ...
   for(j = 0; j < count; j++)
     sds[j] = sdsnew(args[j]);
@@ -594,12 +456,12 @@ static void repl() {
...
@@ -621,16 +483,11 @@ int main(int argc, char **argv) {
