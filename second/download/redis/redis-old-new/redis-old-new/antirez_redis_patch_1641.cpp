...
+unsigned long zslGetRank(zskiplist *zsl, double score, robj *o) {
     ...
     int i;
@@ -287,7 +341,7 @@ unsigned long zslistTypeGetRank(zskiplist *zsl, double score, robj *o) {
 }
...
+zskiplistNode* zslGetElementByRank(zskiplist *zsl, unsigned long rank) {
     ...
     int i;
@@ -343,186 +397,646 @@ static int zslParseRange(robj *min, robj *max, zrangespec *spec) {
     return REDIS_OK;
 }
...
+    llen = zsetLength(zobj);
     ...
     if (start < 0) start = 0;
@@ -535,27 +1049,348 @@ void zremrangebyrankCommand(redisClient *c) {
     }
...
@@ -584,11 +1419,12 @@ void zunionInterGenericCommand(redisClient *c, robj *dstkey, int op) {
     int i, j, setnum;
...
@@ -606,24 +1442,24 @@ void zunionInterGenericCommand(redisClient *c, robj *dstkey, int op) {
     }
...
@@ -664,95 +1500,109 @@ void zunionInterGenericCommand(redisClient *c, robj *dstkey, int op) {
         }
     }
...
@@ -766,16 +1616,13 @@ void zinterstoreCommand(redisClient *c) {
 }
...
+    int rangelen;
     ...
     if ((getLongFromObjectOrReply(c, c->argv[2], &start, NULL) != REDIS_OK) ||
         (getLongFromObjectOrReply(c, c->argv[3], &end, NULL) != REDIS_OK)) return;
@@ -787,13 +1634,11 @@ void zrangeGenericCommand(redisClient *c, int reverse) {
         return;
     }
...
+    llen = zsetLength(zobj);
     ...
     if (end < 0) end = llen+end;
     if (start < 0) start = 0;
@@ -807,23 +1652,68 @@ void zrangeGenericCommand(redisClient *c, int reverse) {
     if (end >= llen) end = llen-1;
...
@@ -839,17 +1729,24 @@ void zrevrangeCommand(redisClient *c) {
...
@@ -877,101 +1774,132 @@ void genericZrangebyscoreCommand(redisClient *c, int reverse, int justcount) {
...
@@ -988,66 +1916,103 @@ void zcountCommand(redisClient *c) {
 }
