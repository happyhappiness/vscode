...
-    if (seplen < 1 || len < 0 || tokens == NULL) return NULL;
     if (len == 0) {
         *count = 0;
         return tokens;
@@ -305,62 +487,48 @@ sds *sdssplitlen(char *s, int len, char *sep, int seplen, int *count) {
             sds *newtokens;
...
-
     while(len--) {
         switch(*p) {
         case '\\':
@@ -399,45 +571,89 @@ sds sdscatrepr(sds s, char *p, size_t len) {
             break;
         }
         p++;
-        if (s == NULL) return NULL;
...
@@ -472,54 +704,80 @@ sds *sdssplitargs(char *line, int *argc) {
                     case '"':
                         inq=1;
                         break;
+                    case '\'':
