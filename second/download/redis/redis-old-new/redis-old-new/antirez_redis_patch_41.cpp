...
@@ -917,7 +448,7 @@ int redisvFormatCommand(char **target, const char *format, va_list ap) {
...
@@ -1098,10 +822,10 @@ int redisBufferRead(redisContext *c) {
...
+            if ((errno == EAGAIN && !(c->flags & REDIS_BLOCK)) || (errno == EINTR)) {
                 ...
                 __redisSetError(c,REDIS_ERR_IO,NULL);
@@ -1125,7 +849,7 @@ int redisBufferWrite(redisContext *c, int *done) {
                 sdsfree(c->obuf);
                 c->obuf = sdsempty();
             } else {
...
@@ -1248,7 +984,7 @@ int redisAppendCommandArgv(redisContext *c, int argc, const char **argv, const s
